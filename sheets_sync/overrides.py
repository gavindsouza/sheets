...
def has_permission(doc, ptype, user):
    if (doc.attached_to_doctype == doc.attached_to_name == sheets_sync.SHEETS_SETTINGS) and (
        doc.attached_to_field == sheets_sync.SHEETS_CREDENTIAL_FIELD
    ):
        raise frappe.PermissionError("Not allowed to access")


class SheetsImporter(Importer):
    def update_record(self, doc):
        ...
        existing_doc, updated_doc = self.get_initial_docs(doc, id_field, unique_field)
    ...
    def get_initial_docs(self, doc, id_field, unique_field):
        try:
            lookup = (
                {unique_field.fieldname: doc.get(unique_field.fieldname)}
                if unique_field
                else doc.get(id_field.fieldname)
            )
            existing_doc = frappe.get_doc(self.doctype, lookup)
            updated_doc = frappe.get_doc(self.doctype, lookup)
